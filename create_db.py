...
tables = ['Evaluation', 'Reservation', 'Chambre', 'TypeChambre', 'Prestation', 'Client', 'Hotel']
for t in tables:
    cur.execute(f"DROP TABLE IF EXISTS {t}")

# Créer les tables
cur.execute("CREATE TABLE Hotel (idHotel INTEGER PRIMARY KEY, ville TEXT, pays TEXT, codePostal INTEGER)")
cur.execute("CREATE TABLE Client (idClient INTEGER PRIMARY KEY AUTOINCREMENT, adresse TEXT, ville TEXT, codePostal INTEGER, email TEXT, telephone TEXT, nom TEXT)")
cur.execute("CREATE TABLE Prestation (idPrestation INTEGER PRIMARY KEY, prix INTEGER, description TEXT)")
cur.execute("CREATE TABLE TypeChambre (idTypeChambre INTEGER PRIMARY KEY, nom TEXT, prix REAL)")
cur.execute("CREATE TABLE Chambre (idChambre INTEGER PRIMARY KEY, numero INTEGER, etage INTEGER, balcon BOOLEAN, idTypeChambre INTEGER, idHotel INTEGER)")
cur.execute("CREATE TABLE Reservation (idReservation INTEGER PRIMARY KEY AUTOINCREMENT, dateDebut TEXT, dateFin TEXT, idClient INTEGER, idChambre INTEGER)")
cur.execute("CREATE TABLE Evaluation (idEvaluation INTEGER PRIMARY KEY, dateEvaluation TEXT, note INTEGER, commentaire TEXT, idClient INTEGER)")
...
cur.executemany("INSERT INTO TypeChambre VALUES (?, ?, ?)", [
    (1, 'Simple', 80),
    (2, 'Double', 120),
])
